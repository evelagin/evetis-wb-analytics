...
--
-- ГРАНИЦА PR-B: слой НЕ меняет смысл существующих KPI. Переименование
--   cost_category 'commission' → 'wb_reward' и перевод основного расхода WB
--   на marketplace_fee_gap_rub — это PR-B2.
--
...
SELECT
  ...
  -- ── 4. Честные семантические метрики (справочные, вне P&L до PR-B2) ─────

  --    4.1 Вознаграждение WB под своим именем.
  COALESCE(SAFE_CAST(REPLACE(s.commission_amount, ',', '.') AS NUMERIC),
           SAFE_CAST(JSON_VALUE(s.raw_json, '$.vw') AS NUMERIC))
    AS wb_reward_rub,

  --    4.2 Native-комиссия по модели REV2.4 — СПРАВОЧНАЯ метрика.
  ...
  --        Authoritative-расход появится в PR-B2 как
  --        marketplace_fee_gap_rub = retail_price_withdisc_rub − for_pay,
  --        который в проблемном окне остаётся корректным.
  ...
  --    4.3 Quality-контракт для 4.2. Детектор размерности сделан по данным,
  ...
  --    4.4 Эквайринг — отдельная метрика для аналитики.
  --        ⚠️ УЖЕ ВХОДИТ в спред srev − for_pay вместе с комиссией.
  --        Это компонент декомпозиции, а не дополнительный расход:
  --        вычитать его из маржи вторым разом нельзя.
