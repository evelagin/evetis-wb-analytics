...
SELECT
  ...
  -- ── 0b. Баллы программы лояльности (Stage 3.1G) ─────────────────────────
  --    WB удерживает/возвращает баллы отдельной величиной cashbackAmount и
  --    учитывает её при расчёте выплаты продавцу. В FACT_FINANCE поля нет,
  --    поэтому оно выводится здесь — там же, где живёт весь разбор JSON.
  --    🔴 Знак источника СОХРАНЯЕТСЯ: отрицательное значение = баллы удержаны
  --    (и потому увеличивают выплату при вычитании). Ни ABS, ни инверсии.
  SAFE_CAST(JSON_VALUE(s.raw_json, '$.cashbackAmount') AS NUMERIC)
                                                              AS loyalty_points_rub,
