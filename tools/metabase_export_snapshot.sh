...
while IFS= read -r rel; do
  src="$rel"
  # Файла может ещё не быть в репозитории, если объект добавлен в манифест
  # этим же прогоном (новая карточка). Тогда считаем сумму по свежему
  # экспорту из временного каталога — он и станет этим файлом.
  # В режиме записи файл уже скопирован, поэтому ветка не срабатывает.
  # Fail-closed сохранён: подмена возможна ТОЛЬКО для объекта, реально
  # выгруженного в этом прогоне; для любого другого отсутствующего файла
  # скрипт по-прежнему прекращает работу.
  [ -f "$src" ] || src="$TMP/$(basename "$rel")"
  [ -f "$src" ] || die "манифест ссылается на отсутствующий файл: $rel"
  jq --arg k "$rel" --arg v "$(shasum -a 256 "$src" | cut -d' ' -f1)" '.[$k]=$v' "$SHAS" > "$SHAS.n"
  mv "$SHAS.n" "$SHAS"
