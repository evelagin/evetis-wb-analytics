...
- **Реклама T8 ↔ T9:** `Σ fullstats.sum` против `Σ updSum`. Это **только API-сверка**. Сходимость (`|Δ| ≤ порог`) означает лишь согласованность API-данных; **финальный SKU-факт рекламы требует сверки с кабинетом WB** и порог в коде — предварительный. До этого реклама по SKU остаётся оценкой.
...
*Тестовый контур соответствует политике проекта: production-код не меняется, боевые листы не затрагиваются, raw JSON — только в Drive, T8/T9 — только API-сверка; финальный SKU-факт рекламы возможен только после сверки с кабинетом WB. COGS в WB API не ищется, Excel/Drive не используется как источник факта.*
