 ...
 *
 * ЗАПУСК — ЧЕТЫРЕ ФУНКЦИИ, по отдельности:
...
function p4Inventory_(fromIdx, toIdx) {
  ...
  // 🔴 Вердикт ЛОКАЛЕН для обработанного диапазона. Глобальный ответ получается
  // только сложением всех прогонов: ноль в одном диапазоне ничего не говорит о
  // соседнем. Формулировки ниже это проговаривают явно.
  if (!firstPositive) {
    p4Log_('  ВЫВОД ПО ДИАПАЗОНУ: положительных accepted здесь нет — граница в этом куске не наблюдается.');
  } else if (zerosAfter === 0) {
    p4Log_('  ВЫВОД ПО ДИАПАЗОНУ: нулей после ' + firstPositive + ' в ЭТОМ диапазоне нет.');
    p4Log_('     Это НЕ значит, что граница чистая глобально — соседние диапазоны надо сложить.');
  } else {
    p4Log_('  🔴 ВЫВОД ПО ДИАПАЗОНУ: после ' + firstPositive + ' встречается ещё ' + zerosAfter +
           ' состоявшихся поставок с accepted = 0.');
    p4Log_('     Одного такого случая достаточно, чтобы чистой временной границы НЕ было');
    p4Log_('     во всей выборке: поле объясняется не только датой.');
  }
